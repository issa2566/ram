@echo off
chcp 65001 > nul
cls

echo ═══════════════════════════════════════════════════════════
echo     🚀 تشغيل خادم Node.js مع PostgreSQL
echo ═══════════════════════════════════════════════════════════
echo.

REM التحقق من وجود Node.js
where node >nul 2>nul
if %ERRORLEVEL% NEQ 0 (
    echo ❌ Node.js غير مثبت!
    echo 💡 قم بتثبيت Node.js من: https://nodejs.org/
    echo.
    pause
    exit /b 1
)

REM التحقق من وجود node_modules
if not exist "node_modules\" (
    echo 📦 تثبيت المكتبات المطلوبة...
    call npm install
    if %ERRORLEVEL% NEQ 0 (
        echo ❌ فشل تثبيت المكتبات!
        pause
        exit /b 1
    )
    echo ✅ تم تثبيت المكتبات بنجاح
    echo.
)

REM اختبار الاتصال بقاعدة البيانات
echo 🔍 اختبار الاتصال بقاعدة البيانات...
call npm run test:db
if %ERRORLEVEL% NEQ 0 (
    echo.
    echo ⚠️  تحذير: فشل الاتصال بقاعدة البيانات
    echo 💡 تأكد من:
    echo    1. تشغيل PostgreSQL
    echo    2. إنشاء قاعدة البيانات testdb
    echo    3. تنفيذ ملف database.sql
    echo.
    echo هل تريد المتابعة رغم ذلك؟ (Y/N)
    set /p continue=
    if /i not "%continue%"=="Y" exit /b 1
)

echo.
echo ═══════════════════════════════════════════════════════════
echo 🚀 تشغيل الخادم...
echo ═══════════════════════════════════════════════════════════
echo.
echo 📍 الخادم سيعمل على: http://localhost:3000
echo 🛑 لإيقاف الخادم، اضغط: Ctrl+C
echo.

REM تشغيل الخادم
call npm start

