...
const FamillesPiecesSectionCompact = () => {
  ...
  const handleImageUpload = async (subcategoryName: string, file: File, familyName?: string) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000/api'}/subcategories/upload-image`, {
        method: 'POST',
        body: formData,
        cache: 'no-store',
        headers: {
  ...
  const handleSubmitNewSubcategory = async () => {
    ...
    try {
      ...
      if (!uploadResult.success) {
        throw new Error(uploadResult.message || "Échec de l'upload de l'image");
      }
                ...
                <button
                  ...
                  className={`group bg-white shadow-sm border rounded-lg sm:rounded-xl p-2 sm:p-4 md:p-5
                              ...
                              ${isActive 
                                ...
                                : 'border-gray-100 hover:shadow-lg hover:border-orange-200 hover:scale-[1.02]'
                              }`}
                >
                  {/* Icon */}
