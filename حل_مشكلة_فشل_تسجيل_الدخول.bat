@echo off
chcp 65001 > nul
cls

echo ═══════════════════════════════════════════════════════════
echo     🔧 حل مشكلة "فشل في تسجيل الدخول"
echo ═══════════════════════════════════════════════════════════
echo.

echo 🛑 الخطوة 1: إيقاف جميع عمليات Node.js القديمة...
taskkill /f /im node.exe >nul 2>&1
timeout /t 2 /nobreak > nul
echo ✅ تم
echo.

echo 📁 الخطوة 2: الانتقال لمجلد Backend...
cd /d "%~dp0backend"

if not exist "server.js" (
    echo ❌ خطأ: server.js غير موجود!
    pause
    exit /b 1
)

echo ✅ server.js موجود
echo.

echo 📦 الخطوة 3: تثبيت المكتبات...
call npm install
echo ✅ تم
echo.

echo 🔧 الخطوة 4: تثبيت bcrypt...
call npm install bcrypt
echo ✅ تم
echo.

echo ═══════════════════════════════════════════════════════════
echo     🔍 تشخيص مشكلة PostgreSQL
echo ═══════════════════════════════════════════════════════════
echo.

echo 💡 كلمات المرور الشائعة لـ PostgreSQL:
echo    1. postgres (الأكثر شيوعاً)
echo    2. admin
echo    3. password
echo    4. (فارغة - اضغط Enter فقط)
echo.

echo 🧪 اختبار الاتصال بـ PostgreSQL...
psql -U postgres -c "SELECT version();" >nul 2>&1

if %ERRORLEVEL% EQU 0 (
    echo ✅ PostgreSQL يعمل ويمكن الاتصال به!
    echo.
    
    echo 📊 التحقق من قاعدة البيانات testdb...
    psql -U postgres -c "SELECT datname FROM pg_database WHERE datname='testdb';" >nul 2>&1
    
    if %ERRORLEVEL% EQU 0 (
        echo ✅ قاعدة البيانات testdb موجودة
    ) else (
        echo ⚠️ قاعدة البيانات testdb غير موجودة
        echo 📝 إنشاء قاعدة البيانات...
        psql -U postgres -c "CREATE DATABASE testdb;" >nul 2>&1
        
        if %ERRORLEVEL% EQU 0 (
            echo ✅ تم إنشاء قاعدة البيانات testdb
            
            echo 📋 إنشاء الجداول...
            psql -U postgres -d testdb -f database.sql >nul 2>&1
            
            if %ERRORLEVEL% EQU 0 (
                echo ✅ تم إنشاء الجداول بنجاح
            ) else (
                echo ⚠️ تحذير: قد تكون هناك مشكلة في إنشاء الجداول
            )
        ) else (
            echo ❌ فشل في إنشاء قاعدة البيانات
        )
    )
) else (
    echo ❌ لا يمكن الاتصال بـ PostgreSQL
    echo.
    echo 💡 الحلول المقترحة:
    echo    1. تأكد من تشغيل PostgreSQL
    echo    2. جرب كلمات مرور مختلفة في backend\db.js
    echo    3. افتح psql يدوياً واختبر الاتصال
    echo.
    echo 🔧 تعديل كلمة المرور:
    echo    - افتح ملف: backend\db.js
    echo    - السطر 8: password: 'كلمة_المرور_الصحيحة'
    echo.
)

echo.
echo ═══════════════════════════════════════════════════════════
echo     🚀 تشغيل Backend
echo ═══════════════════════════════════════════════════════════
echo.

echo 📍 الخادم سيعمل على: http://69.169.108.182:3000
echo.
echo ⚠️ لا تغلق هذه النافذة!
echo.
echo ═══════════════════════════════════════════════════════════
echo.

node server.js

echo.
echo ❌ الخادم توقف!
pause
