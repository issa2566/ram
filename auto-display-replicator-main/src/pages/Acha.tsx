...
const Acha = () => {
  ...
  const handleAddToDashboard = async () => {
    ...
    try {
      ...
      // Build dashboard product object with all required fields including quantity
      // Ensure quantity comes from the loaded Acha product
      const dashboardProduct = {
        id: String(product.id),
        product_id: String(product.id),
        name: product.name || product.sub_id || subId || '',
        reference: product.product_references?.[0] || "",
        price: finalPrice,
        original_price: Number(product.price || 0),
        promotion_price: Number(product.promotion_price || product.price || 0),
        promotion_percentage: Number(product.promotion_percentage || 0),
        image: selectedImageBase64,
        quantity: product.quantity || 0
      };

      // AUTO-CHECK: Print the full object before sending it
      console.log('📤 FULL dashboardProduct object being sent:', JSON.stringify(dashboardProduct, null, 2));
      console.log('🔢 Quantity value:', dashboardProduct.quantity, 'Type:', typeof dashboardProduct.quantity);

      const response = await addDashboardProduct(dashboardProduct);

      // Check for duplicate
      if (response.duplicate === true) {
        alert("Product already exists in dashboard");
        toast({
          title: "Information",
          description: "Ce produit est déjà dans le tableau de bord.",
        });
        return;
      }
      ...
      if (errorMessage.includes('exists') || errorMessage.includes('already')) {
        ...
        toast({
          title: "Erreur",
          description: "Impossible d'ajouter le produit au tableau de bord.",
          variant: "destructive",
        });
      }
    }
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      console.log('📦 Frontend: Order payload being sent:', JSON.stringify(orderData, null, 2));
                  ...
                  return hasPromo ? (
                    <>
                      ...
                      <div className="flex justify-between items-center mt-2 pt-2 border-t border-orange-300">
                        ...
                        <span className="text-base sm:text-xl font-bold text-orange-600">
                          {(promoPrice * orderForm.quantite).toFixed(3)} DT
                        </span>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-gray-700 text-xs sm:text-sm">Total estimé:</span>
                        <span className="text-base sm:text-xl font-bold text-orange-600">
                          {(basePrice * orderForm.quantite).toFixed(3)} DT
                        </span>
                      </div>
                    </>
                  );
